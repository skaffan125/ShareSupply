...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $select_name = $conn->prepare("SELECT * FROM `rec` WHERE email = ?");
      $select_name->execute([$name]);
      if($select_name->rowCount() > 0){
         $message[] = 'username already taken!';
      }else{
         $update_name = $conn->prepare("UPDATE `rec` SET email = ? WHERE id = ?");
         $update_name->execute([$name, $admin_id]);
      }
   }

   $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
   $select_old_pass = $conn->prepare("SELECT password FROM `rec` WHERE id = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `rec` SET password = ? WHERE id = ?");
...
<section class="form-container">
   ...
   <form action="" method="POST">
      ...
      <input type="text" name="email" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')" placeholder="<?= $fetch_profile['email']; ?>">
