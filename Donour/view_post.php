...
if(isset($_POST['add_comment'])){
   ...
   $verify_comment = $conn->prepare("SELECT * FROM `comments` WHERE post_id = ? AND R_id = ? AND user_id = ? AND user_name = ? AND comment = ?");
   $verify_comment->execute([$get_id, $admin_id, $user_id, $user_name, $comment]);

   if($verify_comment->rowCount() > 0){
      $message[] = 'comment already added!';
   }else{
      $insert_comment = $conn->prepare("INSERT INTO `comments`(post_id, R_id, user_id, user_name, comment) VALUES(?,?,?,?,?)");
   ...
   <div class="box-container">
      ...
      <form class="box" method="post">
         ...
         <div class="post-admin">
            ...
            <div>
               <a href="author_posts.php?author=<?= $fetch_posts['name']; ?>"><?= $fetch_posts['name']; ?></a></br>
               <a href="../chat2/php-chat-app-main/chat.php?user=<?= $fetch_posts['email']; ?>"><?= $fetch_posts['email'];?></a>
               <div><?= $fetch_posts['date']; ?></div>
            </div>
         </div>
         <a href="../chat2/php-chat-app-main/chat.php?user=<?= $fetch_posts['email']; ?>" class="button">Message</a>
         <?php
            if($fetch_posts['image'] != ''){  
         ?>
           <div class="imgBg">
            <img src="../uploaded_img/<?= $fetch_posts['image']; ?>" alt="bg" class="coverFull">
            
        </div>
        <div class="imgBg">
         <iframe src="../uploaded_pdf/<?= $fetch_posts['pdf']; ; ?>" width="100%" height="600px"> </iframe>
            </div>
         <?php
         }
         ?>
...
<section class="comments-container">
   ...
   <form action="" method="post" class="add-comment">
      <input type="hidden" name="admin_id" value="<?= $fetch_admin_id['R_id']; ?>">
